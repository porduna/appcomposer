            ...
            var findClearedLine = function(start, mult, anchorPos, dimension) {
                ...
                lineCalculators = {
                    perpendicular : function(axis, ss, oss, es, oes) {
                        // FIXME // "with" is not allowed in strict mode
                        /*
                        with (paintInfo) {
                            var sis = {
                        ...
                        */
                        var sis = {
                            ...
                            midLines = {
                                x:[ [ midx, paintInfo.startStubY ], [ midx, paintInfo.endStubY ] ],
                                y:[ [ paintInfo.startStubX, midy ], [ paintInfo.endStubX, midy ] ]
                            },
                            linesToEnd = {
                            ...
                            startToMidToEnd = {
                                x:[ [ paintInfo.startStubX, midy ], [ paintInfo.endStubX, midy ], [ paintInfo.endStubX, paintInfo.endStubY ] ],
                                y:[ [ midx, paintInfo.startStubY ], [ midx, paintInfo.endStubY ], [ paintInfo.endStubX, paintInfo.endStubY ] ]
                            },
                            otherStubs = {
                            ...
                            soIdx = orientations[axis][0], toIdx = orientations[axis][1],
                            _so = paintInfo.so[soIdx] + 1,
                            _to = paintInfo.to[toIdx] + 1,
                            otherFlipped = (paintInfo.to[toIdx] == -1 && (otherStubs[axis][1] < otherStubs[axis][0])) || (paintInfo.to[toIdx] == 1 && (otherStubs[axis][1] > otherStubs[axis][0])),
                            stub1 = stubs[axis][_so][0],
                            stub2 = stubs[axis][_so][1],
                            segmentIndexes = sis[axis][_so][_to];

                        if (paintInfo.segment == segmentIndexes[3] || (paintInfo.segment == segmentIndexes[2] && otherFlipped)) {
                            return midLines[axis];
                        }
                        else if (paintInfo.segment == segmentIndexes[2] && stub2 < stub1) {
                            return linesToEnd[axis];
                        }
                        else if ((paintInfo.segment == segmentIndexes[2] && stub2 >= stub1) || (paintInfo.segment == segmentIndexes[1] && !otherFlipped)) {
                            return startToMidToEnd[axis];
                        }
                        else if (paintInfo.segment == segmentIndexes[0] || (paintInfo.segment == segmentIndexes[1] && otherFlipped)) {
                            return startToEnd[axis];
                        }
                        // end FIXME
		...
		*/
		...
		getUIPosition : function(eventArgs, zoom) {
			...
			if (eventArgs.length == 1) {
				// FIXME
                var ret = { left: eventArgs[0].pageX, top:eventArgs[0].pageY };
			}
			else {
				var ui = eventArgs[1],
				  _offset = ui.offset;
				// FIXME
				var ret = _offset || ui.absolutePosition;
