...
class RepositoryAppCheckUrl(db.Model):
    # TODO: if it the app is deleted, all the historic data is deleted too.
    ...
    repository_app = db.relation("RepositoryApp", backref="check_urls", cascade="delete")
    ...
    def filename_with_proxy(self):
        return '{}_proxy.png'.format(hashlib.md5(self.url.encode('utf8')).hexdigest())

    @property
    def filename_without_proxy(self):
        return '{}_non-proxy.png'.format(hashlib.md5(self.url.encode('utf8')).hexdigest())
...
class RepositoryAppFailure(db.Model):
    ...
    repository_app_check_url = db.relation("RepositoryAppCheckUrl", backref="failures", cascade="delete")
...
class RepositoryAppLanguage(db.Model):
    ...
    repository_app = db.relation("RepositoryApp", backref="languages", cascade="delete")
...
class TranslationExternalSuggestion(db.Model):
    ...
    def __init__(self, engine, human_key, language, origin_language, value):
        ...
        self.human_key_hash = unicode(hashlib.md5(human_key.encode('utf8')).hexdigest())
        self.language = language
